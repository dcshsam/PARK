...
export function useJarvis(): UseJarvisResult {
  ...
  const persistMessage = useCallback(async (message: Omit<JarvisMessageRecord, "id">) => {
    ...
    try {
      ...
      // Keep the conversation usable if browser storage is temporarily unavailable.
    }
  ...
  const confirmPendingInternal = useCallback(
    async (approved: boolean) => {
      ...
      setStatus("thinking");
      try {
        await handleOutcome(await resume(approved));
      } catch (caught) {
  ...
  const handleRequest = useCallback(
    async (text: string) => {
      ...
      setStatus("thinking");
      try {
        await handleOutcome(await runAgentTurn(request, history, buildCtx()));
      } catch (caught) {
        setError(caught instanceof Error ? caught.message : "Something went wrong");
        setStatus("idle");
      }
  ...
  return {
    ...
    pending,
    error,
    submitText: (text: string) => void handleRequest(text),
